...
function Dashboard({
  ...
  const triggerMealAnalysis = useCallback(
    async (mealLogId: string) => {
      try {
        const idToken = await user.getIdToken();
        const response = await fetch(`/api/meal-logs/${mealLogId}/analyze`, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            console.error("[triggerMealAnalysis] Failed to parse error response", parseError);
          }

          console.error("[triggerMealAnalysis] Request failed", {
            mealLogId,
        ...
        console.error("[triggerMealAnalysis] Failed to call meal analysis endpoint", error, {
          mealLogId,
        });
      }
    },
  ...
  const attemptSaveMeal = useCallback(
    async (
      ...
      try {
        ...
        void triggerMealAnalysis(savedDocRef.id);
